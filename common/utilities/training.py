...
def train(project, env, prop_type=''):
    ...
    if dataset != None:
        ...
        # Initialize agent weights from loaded ensemble if available
        # This allows retraining to continue from existing weights
        if hasattr(dataset, 'initialize_agent_weights'):
            dataset.initialize_agent_weights(project.agent)
        # Behavioral cloning
        training_epoch, train_accuracy, test_accuracy, train_loss, test_loss = project.agent.behavioral_cloning(env, data, project.command_line_arguments['bc_epochs'], project.command_line_arguments.get('accuracy_threshold', 100.0))
