...
class GlobalUnstructuredPruner(Interpreter):
    ...
    def interpret(self, env, m_project, model_checking_info):
        ...
        parameters_to_prune = [(m_project.agent.q_eval.layers[self.layer_index], 'weight'),(m_project.agent.q_eval.layers[self.layer_index+1], 'weight')]
        prune.global_unstructured(parameters_to_prune, pruning_method=prune.L1Unstructured, amount=self.percentage)
