...
class L1UnstructuredPruner(Interpreter):
    ...
    def interpret(self, env, m_project, model_checking_info):
        original_weights = get_original_weights(m_project.agent.q_eval,self.layer_index)
        ...
        prune.l1_unstructured(m_project.agent.q_eval.layers[self.layer_index], name="weight", amount=self.percentage)
        ...
        if self.file_name != "":
            ...
            f.close()
        #print("Run ID: " + run_id)
        #heatmap(m_project.agent.q_eval, original_weights, self.layer_index)
