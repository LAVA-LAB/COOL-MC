...
class PPOAgent(Agent):
    ...
    def behavioral_cloning(self, env, data: dict, epochs: int = 100, accuracy_threshold: float = 100.0):
        ...
        print(f"\nBehavioral Cloning Pre-Training for PPO")
        ...
        temp_model_dir = 'tmp_bc_ppo_best_model'
        os.makedirs(temp_model_dir, exist_ok=True)
        best_model_checkpoint = os.path.join(temp_model_dir, 'best_ppo_policy.chkpt')
        ...
        if os.path.exists(best_model_checkpoint):
            ...
            print(f"\n{'='*70}")
            print(f"PPO Behavioral Cloning Pre-Training Complete!")
            print(f"{'='*70}")
        ...
        print(f"PPO fine-tuning settings:")
